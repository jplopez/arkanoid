start_scene = class:new({
  ...
  update = function(self)
    ...
    if begin_anim_title then
      ...
      -- delay to begin title anim
      _t=(_t+1)%8
      if(_t==0) begin_anim_title=true  
    end
  end,
  ...
  draw = function(self) 
    cls(1)
    palt(1)
    self.stamp_title(0,self.ty,128,32)
    self.stamp_pico(0,15,16,self.ph)
  ...
  stamp_title=function(dx,dy,tw,th)
    sspr(0,64,128,32,dx,dy,tw,th,false, false)
  end,
  
  stamp_pico=function(dx,dy,tw,th)
    -- Letter P
